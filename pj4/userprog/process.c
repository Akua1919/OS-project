...
struct exec_info
{
    ...
    struct semaphore wait;         /* wait until load has been done */
    struct wait_status *wait_status;    /* child process */
    bool load;                       /* loaded or not  */
    struct dir *cwd;                    /* cwd */
};
...
process_execute (const char *file_name)
{
  ...
  exec.file = file_name;
  sema_init (&exec.wait, 0);
  exec.cwd = thread_current ()->cwd;

  /* Create a new thread to execute FILE_NAME. */
  strlcpy (thread_name, file_name, sizeof thread_name);
  char *save;
  strtok_r (thread_name, " ", &save);
...
start_process (void *exec_)
{
  ...
  success = load (exec->file, &if_.eip, &if_.esp);

  /* Allocate wait_status and initial. */
  if (success)
    {
...
/* releases and if it is now unreferenced,frees it. */
static void
release_child (struct wait_status *status)
{
  int record;
  lock_acquire (&status->lock);
  record = --status->ref_cnt;
...
process_wait (tid_t child_tid UNUSED )
{
  ...
  for (elem = list_begin (&cur->children); elem != list_end (&cur->children);
       elem = list_next (elem))
    {
      struct wait_status *status = list_entry (elem, struct wait_status, elem);
      if (status->tid == child_tid)
        {
          list_remove (elem);
          sema_down (&status->dead);
          int exit;
          exit = status->exit_code;
          release_child (status);
          return exit;
        }
    }
  return -1;
...
process_exit (void)
{
  ...
  {
      struct wait_status *status = cur->wait_status;
      printf ("%s: exit(%d)\n", cur->name, status->exit_code);
      sema_up (&status->dead);
      release_child (status);
  }
...
/* Pushes the SIZE bytes in BUF onto the stack in KPAGE */
static void *
               ...
               void **esp)
{
  ...
  if (cmd_line_copy == NULL || push (kpage, &ofs, &null, sizeof null) == NULL)
    return false;
